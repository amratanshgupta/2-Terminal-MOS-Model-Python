...
class Total_charge_VGB_Surface_Potential(Surface_Potential_To_VGB):
    ...
    def Plot_Q_vs_Surface_Potential(self):
        ...
        plt.plot(self.Surface_Potential_Qinv_list, self.Qinv, 'r')
        plt.plot(self.Surface_Potential_Qdep.keys(), self.Qdep, 'g')
        plt.plot(self.phi_vgb_map.keys(), self.Qtotal, 'k', linestyle='--')
        plt.title(r'Total Semiconductor Charge($Q_{SC}$) vs  Surface Potential for 2-Terminal MOS')
