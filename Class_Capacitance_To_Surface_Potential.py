...
class Capacitance_To_Surface_Potential(Total_charge_VGB_Surface_Potential):
    ...
    def __init__(self, tox, gamma, Vgb_max, Na=1e17):
        ...
        eps = 8.854e-14 * (11.8)
        q = 1.6e-19
        Total_charge_VGB_Surface_Potential.__init__(self, tox, gamma, Vgb_max, Na=1e17)
    ...
    def Dep_Capacitance(self):
        ...
        for k in self.phi_vgb_map.keys():
            if k > 0.05:
                first = 1 / (m.sqrt(abs(k + vt * (m.exp((k - (2 * self.phi_f)) / vt)))))
    ...
    def Inv_Capacitance(self):
        ...
        for k in self.phi_vgb_map.keys():
            if k > 0.05:
                ...
                first = 1 / (m.sqrt(abs(k + vt * (m.exp((k - (2 * self.phi_f)) / vt)))))
                # print(first)
                C_inv = first * (m.sqrt(2 * q * eps * self.Na)) * m.exp((k - (2 * self.phi_f)) / vt) / 2
    ...
    def Plot_C_vs_Surface_Potential(self):
        plt.semilogy(self.Surface_Potential_Cinv.keys(), self.Cinv, 'r', linestyle='--')
        plt.semilogy(self.Surface_Potential_Cdep.keys(), self.Cdep, 'g', linestyle='--')
        plt.semilogy(self.Surface_Potential_Csemiconductor.keys(), self.Csemiconductor, 'k', linestyle=':')
        plt.semilogy(self.Surface_Potential_Ctotal.keys(), self.Ctotal, 'b', linestyle='-.')
        plt.axhline(y=self.Cox, color='#E111B7', linestyle='-.')
        plt.title(r'Capacitance($C_{}$) vs  Surface Potential for 2-Terminal MOS')
        plt.ylabel('Capacitance (F)')
        plt.xlabel('Surface Potential (V)')
        plt.legend([r'$C_{inv}$', r'$C_{dep}$', r'$C_{semiconductor}$', r'$C_{Total}$', r'$C_{ox}$'])
        plt.show()
